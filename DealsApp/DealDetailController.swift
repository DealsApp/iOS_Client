...
//  DealsApp
//
//  Created by Chen, David on 9/20/16.
...
class DealDetailController: UIViewController, BTDropInViewControllerDelegate {
    ...
    @IBAction func sendRedeem(sender: UIButton!) {
//        performSegueWithIdentifier("presentPaymentViewController", sender: self)
    }
...
//        case "presentPaymentViewController":    //Case 1: Storyboard Segues references another Storyboard which requires a UINavigationController.
//            guard let navigationController = segue.destinationViewController as? UINavigationController,
//                let paymentViewController = navigationController.topViewController as? PaymentViewController else {
//                    fatalError("Unexpected view hierarchy")
//            }
//             paymentViewController.deal = deal
            
//        case "presentPaymentViewController":  //Case 2: Storyboard Seques references another ViewController directly.
//            guard let paymentViewController = segue.destinationViewController as? PaymentViewController else {
//                fatalError("Unexpected view hierarchy")
//            }
           
            //send deal["categoryID"] to server to update preferences.
            print ("deal_id: \(deal["_id"]!)")
            let dealUrlID = NSURL(string: "http://smat-server.herokuapp.com/deals/redeem/\(deal["_id"]!)")!
            let request = NSMutableURLRequest(URL: dealUrlID)
//            request.HTTPBody = "\(deal["_id"]!)".dataUsingEncoding(NSUTF8StringEncoding)
